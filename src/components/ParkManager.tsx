...
export default function ParkManager() {
  // TODO: Kết ná»‘i dữ liệu công viên và mảng xanh từ Supabase
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#f5f7fa] custom-scrollbar">
      <div className="max-w-[1600px] mx-auto p-6 flex flex-col gap-5">
        ...
        {/* Header */}
        <div>
          <h2 className="text-xl font-semibold text-[#333]">Công viên & Mảng xanh</h2>
          <p className="text-sm text-[#999] mt-0.5">
            {PARKS.length} công viên Â· {GREEN_AREAS.length} mảng xanh đang được quản lý
          </p>
        </div>
                ...
                <div key={park.id} id={`park-card-${park.id}`} className="card overflow-hidden">
                  ...
                  {assets.length > 0 && (
                    <div className="px-5 py-3 space-y-3">
                      {cayTrong.length > 0 && <AssetGroup title="ðŸŒ± Cây trồng" assets={cayTrong} />}
                      {haTang.length > 0 && <AssetGroup title="ðŸ—ï¸ Hạ tầng" assets={haTang} />}
                      {thietBi.length > 0 && <AssetGroup title="âš™ï¸ Thiết bị" assets={thietBi} />}
                    </div>
                  )}
